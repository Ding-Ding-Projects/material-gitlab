...
try {
  ...
  const receipt = { schemaVersion: 2, id: args.id, kind: 'side-by-side', status: 'verified', sourceCommit: String(args.commit), tuple, tupleHash: hashJson(tuple), artifact: { sha256: hash(output) }, inputs: { reference: { path: path.relative(ROOT, referencePath).replaceAll('\\', '/'), sha256: hash(referencePath) }, built: { path: path.relative(ROOT, builtPath).replaceAll('\\', '/'), sha256: hash(builtPath) } }, dimensions: { width, height }, tool: 'tools/design-reference/scripts/side-by-side.mjs' };
