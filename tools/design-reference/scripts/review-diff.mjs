...
try {
  ...
  if (path.isAbsolute(String(args.diff))) throw new Error('diff path must be relative to the repository root');
  const diffPath = path.resolve(ROOT, String(args.diff));
  if (!diffPath.startsWith(`${ROOT}${path.sep}`) || !fs.existsSync(diffPath)) throw new Error('diff record is missing or escapes the repository root');
