...
function config(): DeploymentConfig {
  ...
  return { projectPath, image, target: { kind: selected, label: targetCopy[selected].title, distro, host, user }, environment: { GITLAB_OMNIBUS_CONFIG: 'vault://gitlab/config' }, secretRefs: ['gitlab/config'] };
}
...
function render(): void {
  ...
  appRoot.innerHTML = `<div class="shell">
    ...
    <section class="content"><header class="topbar"><div><p class="kicker">Configuration workspace</p><h1>Plan a safe deployment</h1><p class="subtitle">Choose an endpoint, supply bounded inputs, and review the generated command plan before an approved executor receives it.</p></div><div class="status ${phase === 'cancelled' ? 'status-cancelled' : 'status-preview'}">${phase === 'cancelled' ? '● Review cancelled' : '● Preview only'}</div></header>
      ...
      <div class="fields"><label class="field">Project directory<input id="project-path" type="text" value="${escapeHtml(input('project-path', './deploy'))}" autocomplete="off" /><small>Relative or absolute path accepted by the bounded planner.</small></label><label class="field">Container image<input id="image" type="text" value="${escapeHtml(input('image', 'gitlab/gitlab-ee:latest'))}" autocomplete="off" /><small>Image reference is validated before it enters the plan.</small></label><div id="wsl-fields" ${selected === 'wsl2' ? '' : 'hidden'}><label class="field">WSL2 distribution<input id="distro" type="text" value="${escapeHtml(input('distro', 'Ubuntu'))}" autocomplete="off" /></label></div><div id="ssh-fields" ${selected === 'ssh-docker' ? '' : 'hidden'}><label class="field">SSH user<input id="user" type="text" value="${escapeHtml(input('user', 'deploy'))}" autocomplete="off" /></label><label class="field">SSH host<input id="host" type="text" value="${escapeHtml(input('host', ''))}" placeholder="docker.example.internal" autocomplete="off" /></label><label class="field">SSH port<input id="port" type="number" value="22" min="1" max="65535" /></label></div></div><p class="hint boundary">🔒 Secrets stay as vault references. This preview never reads, prints, stores, or transmits secret values.</p></section>
  ...
  appRoot.querySelector<HTMLButtonElement>('#preview')?.addEventListener('click', () => { try { const plan = buildCommandPlan(config()); planText = plan.steps.map((step) => `<article class="step"><div class="step-title"><span>${step.id}</span><strong>${escapeHtml(step.title)}</strong></div><code>${escapeHtml(formatStep(step))}</code><p class="hint">${escapeHtml(step.rationale)}${step.sensitive ? ' Secret values remain outside this display.' : ''}</p></article>`).join(''); phase = 'review'; errorText = ''; } catch (error) { errorText = error instanceof Error ? error.message : 'The plan could not be built.'; } render(); });
