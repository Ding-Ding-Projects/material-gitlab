...
RSpec.describe API::Ai::DuoWorkflows::Workflows, :with_current_organization, feature_category: :duo_agent_platform do
  ...
  describe 'GET /ai/duo_workflows/ws' do
    ...
    context 'when user is authenticated' do
      # Listed literally so adding a `readOnlyHint` tool fails here and the new
      # pre-approval gets a deliberate review.
      let(:mcp_preapproved_tools) do
        %w[
          ...
          get_pipeline get_pipeline_jobs get_job_log
          get_issue get_workitem_notes get_merge_request_notes
          get_saved_view_work_items search_labels
          get_mcp_server_version get_merge_request_conflicts
          get_work_item_types list_wiki_pages
          list_merge_requests gitlab_merge_request_search list_pipelines
        ]
      end
