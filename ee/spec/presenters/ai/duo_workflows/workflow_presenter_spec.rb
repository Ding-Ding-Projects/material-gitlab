...
RSpec.describe ::Ai::DuoWorkflows::WorkflowPresenter, feature_category: :duo_agent_platform do
  ...
  describe 'first_checkpoint' do
    ...
    it 'forwards checkpoint_ns to scope the lookup to one nested subgraph invocation' do
      nested_checkpoint = instance_double(::Ai::DuoWorkflows::Checkpoint)
      checkpoints_relation = double

      allow(workflow).to receive(:checkpoints).and_return(checkpoints_relation)
      allow(checkpoints_relation).to receive(:earliest).with(checkpoint_ns: 'delegation:task-1')
        .and_return(nested_checkpoint)

      expect(presenter.first_checkpoint(checkpoint_ns: 'delegation:task-1')).to eq(nested_checkpoint)
    end
  end
  ...
  describe 'latest_checkpoint' do
    ...
    it 'forwards checkpoint_ns to scope the lookup to one nested subgraph invocation' do
      ...
      allow(workflow).to receive(:checkpoints).and_return(checkpoints_relation)
      allow(checkpoints_relation).to receive(:latest).with(checkpoint_ns: 'delegation:task-1')
        .and_return(nested_checkpoint)

      expect(presenter.latest_checkpoint(checkpoint_ns: 'delegation:task-1')).to eq(nested_checkpoint)
    end
  end
