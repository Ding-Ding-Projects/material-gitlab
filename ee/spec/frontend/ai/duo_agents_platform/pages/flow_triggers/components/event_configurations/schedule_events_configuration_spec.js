...
describe('ScheduleEventsConfiguration', () => {
  ...
  describe('frequency group spacing', () => {
    it('drops the bottom margin when the frequency has no contextual fields', () => {
      createWrapper({ value: filterFor({ frequency: SCHEDULE_FREQUENCY_EVERY_15_MINUTES }) });

      expect(findFormGroup().classes()).toContain('gl-mb-0');
    });

    it('keeps a bottom margin when contextual fields follow', () => {
      createWrapper({ value: filterFor({ frequency: SCHEDULE_FREQUENCY_DAILY }) });

      expect(findFormGroup().classes()).not.toContain('gl-mb-0');
    });
  });
