...
describe('FlowTriggerForm', () => {
  ...
  describe('merge_request initial value handling', () => {
    it('presents a stored merge_request_ready trigger as merge_request with Ready selected', async () => {
      ...
      expect(findEventsField().props('eventTypes')).toEqual(['merge_request']);
      expect(findEventsField().props('filter')).toEqual({
  ...
  describe('FlowTriggerEventsField integration', () => {
    ...
    it('submits a new code_conflict selection as event_type 5 rather than merge_request (6)', async () => {
      await findEventsField().vm.$emit('update:event-types', ['merge_request']);
      await findEventsField().vm.$emit('update:filter', {
