...
describe('SecretsTable component', () => {
  ...
  const findEmptyState = () => wrapper.findByTestId('secrets-list-empty-state');
  ...
  describe('when OpenBao is unhealthy', () => {
    ...
    it('does not show the new secret button in the empty state', () => {
  ...
  describe('when entitlement state is trial eligible', () => {
    beforeEach(async () => {
      await createComponent({ entitlement: { state: ENTITLEMENT_STATE_TRIAL_ELIGIBLE } });
    });
