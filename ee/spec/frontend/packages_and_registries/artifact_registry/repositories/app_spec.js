...
describe('ArtifactRegistryRepositoriesApp', () => {
  ...
  const createComponent = async (initialPath = '/') => {
    ...
    wrapper = mountExtended(App, {
      router,
      // Each @client document keeps its own reduced `organization { id }` sibling, so
      // the two reads need a handler each.
      apolloProvider: createMockApollo(
        ...
        ],
        {
          Organization: {
            ...
            artifactRegistryRepository: () => mockRepository,
          },
        },
      ),
      provide: {
  ...
  describe('client-side route changes', () => {
    it('moves focus to the newly rendered view', async () => {
      ...
      // Focus starts outside the shell, so the assertion below shows that the
      // route change is what moved it.
      document.body.focus();
