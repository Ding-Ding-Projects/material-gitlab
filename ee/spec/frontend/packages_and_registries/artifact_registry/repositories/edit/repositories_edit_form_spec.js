...
describe('ArtifactRegistryRepositoriesEditForm', () => {
  ...
  let repositoryResolver;
  let updateMutationResolver;
  let toastShow;
  ...
  const createComponent = async ({ name = 'my-repository' } = {}) => {
    ...
    const mockApollo = createMockApollo(
      [[getRepositoryQuery, mockOrganizationHandler()]],
      {
        Organization: { artifactRegistryRepository: repositoryResolver },
        Mutation: { artifactRegistryRepositoryUpdate: updateMutationResolver },
      },
  ...
  beforeEach(() => {
    storedRepository = { ...mockRepository };

    repositoryResolver = jest.fn(() => ({ ...storedRepository }));
    updateMutationResolver = jest.fn((_, { input }) => {
      storedRepository = {
        ...storedRepository,
  ...
  describe('once the repository resolves', () => {
    ...
    it('reads the repository named in the route', () => {
  ...
  describe('when the repository does not resolve', () => {
    beforeEach(async () => {
      repositoryResolver.mockResolvedValue(null);
  ...
  describe('when the read fails', () => {
    beforeEach(async () => {
      repositoryResolver.mockRejectedValue(new Error('Service unavailable'));
      await createResolvedComponent();
    });
  ...
  describe('when the mutation returns a recoverable error', () => {
    beforeEach(async () => {
      updateMutationResolver.mockResolvedValue(mockErrorPayload(['Repository not found.']));
      await createResolvedComponent();
      await submitForm();
    });
    ...
    it('drops them when the form is submitted again', async () => {
      updateMutationResolver.mockRejectedValue(new Error('Service unavailable'));
      await submitForm();
