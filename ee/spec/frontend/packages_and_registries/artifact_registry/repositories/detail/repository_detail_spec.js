...
// The page hands the whole query result down, so spelling this out is what pins the
// document's selection set: an over- or under-selected one fails the deep-equal below.
const mockQueryRepository = {
  ...
  artifactsCount: '3',
  downloadsCount: mockRepository.downloadsCount,
  sizeBytes: mockRepository.sizeBytes,
  createdAt: '2026-05-12T09:24:00Z',
  lastUpdatedAt: mockRepository.lastUpdatedAt,
  createdBy: mockUser,
  updatedBy: mockUser,
};
...
const mockRepositoryWith = (overrides = {}) => {
  const format = overrides.format ?? mockQueryRepository.format;

  return { ...mockQueryRepository, ...ARTIFACT_CONNECTIONS[format], ...overrides };
};
...
describe('ArtifactRegistryRepositoryDetail', () => {
  ...
  // A variable the page leaves out reaches the handler as an absent key, and one it
  // passes as undefined as a key holding undefined. Dropping those makes the variables
  // comparable whole, so an unexpected one fails rather than passing a partial match.
  const lastQueryVariables = (handler) => omitBy(handler.mock.calls.at(-1)[0], isUndefined);

  const createComponent = async ({
    repositoryResolver = jest.fn().mockResolvedValue(mockRepositoryWith()),
    imagesHandler = mockOrganizationHandler(),
    path = '/my-repository',
    query = {},
  } = {}) => {
    ...
    wrapper = shallowMountExtended(RepositoryDetail, {
      ...
      apolloProvider: createMockApollo(
        [
          [getRepositoryDetailQuery, mockOrganizationHandler()],
          [getRepositoryImagesQuery, imagesHandler],
          [getRepositoryPackagesQuery, mockOrganizationHandler()],
        ],
  ...
  it('reads the repository the route names, so a shared URL opens that repository', async () => {
    const repositoryResolver = jest.fn().mockResolvedValue(mockQueryRepository);

    await createResolvedComponent({ repositoryResolver, path: '/payment-core' });
  ...
  describe('when the repository loads', () => {
    ...
    it('hands the header the repository the read returned', () => {
      expect(findHeader().props('repository')).toEqual(mockQueryRepository);
    });

    it('hands the sidebar the same repository, so the two cannot disagree', () => {
      expect(findSidebarComponent().props('repository')).toEqual(mockQueryRepository);
    });
  ...
  describe('the artifact connection', () => {
    it.each([
      ...
      await createResolvedComponent({
        repositoryResolver: jest.fn().mockResolvedValue(mockRepositoryWith({ format })),
      });
    ...
    it('passes a null connection down without taking the whole page with it', async () => {
      await createResolvedComponent({
        repositoryResolver: jest
          .fn()
          .mockResolvedValue(mockRepositoryWith({ format: 'DOCKER', images: null })),
      });
    ...
    it('reads the artifacts of the repository the route names, so the page addresses one throughout', async () => {
      const repositoryResolver = jest.fn().mockResolvedValue(mockRepositoryWith());

      await createResolvedComponent({ repositoryResolver, path: '/payment-core' });

      expect(repositoryResolver).toHaveBeenNthCalledWith(
        2,
        expect.anything(),
        expect.objectContaining({ name: 'payment-core' }),
        expect.anything(),
        expect.anything(),
      );
    });

    it('stops loading when the read fails outright', async () => {
      await createResolvedComponent({
        repositoryResolver: jest
          .fn()
          .mockResolvedValueOnce(mockQueryRepository)
  ...
  // The document to issue follows the repository's format, so reading before the format
  // has landed would issue the packages document for a container repository and then swap
  // documents once it arrives.
  describe('before the repository has resolved', () => {
    it('issues no artifact read while the repository read is still in flight', async () => {
      const repositoryResolver = jest.fn().mockReturnValue(new Promise(() => {}));

      await createResolvedComponent({ repositoryResolver });

      expect(repositoryResolver).toHaveBeenCalledTimes(1);
    });

    it('issues no artifact read for a repository that resolved null', async () => {
      const repositoryResolver = jest.fn().mockResolvedValue(null);
    ...
    it('reads a container repository once, on the document its format names', async () => {
      const repositoryResolver = jest
        .fn()
        .mockResolvedValue(mockRepositoryWith({ format: 'DOCKER' }));

      await createResolvedComponent({ repositoryResolver });

      expect(repositoryResolver).toHaveBeenCalledTimes(2);
  ...
  describe('when the artifact connection comes back empty', () => {
    beforeEach(async () => {
      await createResolvedComponent({
        repositoryResolver: jest
          .fn()
          .mockResolvedValue(mockRepositoryWith({ format: 'DOCKER', images: mockEmptyImagePage })),
      });
    });

    it('hides the sidebar stats even though the buffered count says otherwise', () => {
      expect(mockQueryRepository.artifactsCount).toBe('3');
  ...
  describe('the artifact pager', () => {
    ...
    // after the first page's end cursor, and every other request answers the first,
    // including the walk back from the second page's start cursor.
    const pagedRepositoryResolver = (pages = {}) =>
      jest.fn().mockImplementation((_, { after }) =>
        mockRepositoryWith({
          format: 'DOCKER',
          images:
            after === FIRST_PAGE_END_CURSOR
              ? (pages.second ?? mockSecondImagePage)
              : (pages.first ?? mockFirstImagePage),
        }),
      );

    const pagedComponent = (options = {}) =>
      createResolvedComponent({ repositoryResolver: pagedRepositoryResolver(), ...options });
    ...
    it('hands the pager no page either way for a single-page connection, which hides it', async () => {
      await pagedComponent({
        repositoryResolver: jest
          .fn()
          .mockResolvedValue(mockRepositoryWith({ format: 'DOCKER', images: mockImagePage })),
      });
    ...
    `('requests the $page page the route query names', async ({ query, pageArguments }) => {
      const imagesHandler = mockOrganizationHandler();

      await pagedComponent({ imagesHandler, query });

      expect(lastQueryVariables(imagesHandler)).toStrictEqual({
  ...
  describe('the artifact live region', () => {
    ...
    it('announces again on a page change, so an unchanged message still reaches a reader', async () => {
      await createResolvedComponent({
        repositoryResolver: jest.fn().mockImplementation((_, { after }) =>
          mockRepositoryWith({
            format: 'DOCKER',
  ...
  // The read resolves null both for a repository that does not exist and for one the
  // viewer may not see, so the view has a single outcome to render and never tells the
  // two apart.
  describe('when the repository resolves null', () => {
    beforeEach(async () => {
      await createResolvedComponent({
        repositoryResolver: jest.fn().mockResolvedValue(null),
      });
    });
  ...
  describe('when the detail query fails', () => {
    beforeEach(async () => {
      await createResolvedComponent({
        repositoryResolver: jest.fn().mockRejectedValue(new Error('Artifact Registry is down')),
      });
    });
  ...
  // Nothing on the page moves the route yet, so today the query runs once per mount. The
  // keyset pager changes that: it holds the cursor in the route query, which re-runs this
  // same document on the mounted component, and a failed page followed by browser Back
  // would otherwise leave the alert over a page that loaded.
  describe('when a failed read is followed by a successful one', () => {
    it('renders the repository, so a recovered read is not hidden behind the old error', async () => {
      const repositoryResolver = jest
        ...
        .mockResolvedValue(mockQueryRepository);
  ...
  describe('when a failed artifact read is followed by a successful one', () => {
    it('renders the artifacts, so a recovered read is not hidden behind the old error', async () => {
      const repositoryResolver = jest
        ...
        .mockResolvedValueOnce(mockQueryRepository)
        .mockRejectedValueOnce(new Error('Artifact Registry is down'))
        .mockResolvedValue(mockRepositoryWith());
