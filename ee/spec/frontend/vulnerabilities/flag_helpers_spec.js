...
describe('Vulnerability flag helpers', () => {
  ...
  describe('isSastFpDetectionApplicable', () => {
    ...
    it.each`
      ...
      ${{ vulnerability: { severity: 'MEDIUM' } }}                                                          | ${false}
      ${{ vulnerability: { severity: 'UNKNOWN' } }}                                                         | ${false}
      ${{ vulnerability: { latestFlag: { status: FLAG_STATUSES.IN_PROGRESS, origin: 'ai_sast_fp' } } }}     | ${false}
      ${{ vulnerability: { latestFlag: { status: FLAG_STATUSES.DETECTED_AS_FP, origin: 'ai_sast_fp' } } }}  | ${false}
      ${{ vulnerability: { latestFlag: { status: FLAG_STATUSES.DETECTED_AS_FP, origin: 'manual_user' } } }} | ${true}
      ${{ vulnerability: { latestFlag: { status: FLAG_STATUSES.DISMISSED, origin: 'ai_sast_fp' } } }}       | ${true}
    `('returns $expected when $override', ({ override, expected }) => {
      expect(
        isSastFpDetectionApplicable({
          vulnerability: { ...baseVulnerability, ...(override.vulnerability ?? {}) },
    ...
    // The API delivers the upper-cased GraphQL enum name, but severity reaches
    // some callers already normalized, so both spellings must match.
    it.each(['CRITICAL', 'critical', 'HIGH', 'high'])(
      'matches the %s severity spelling',
      (severity) => {
        expect(
          isSastFpDetectionApplicable({
  ...
  describe('eligibleSastFpVulnerabilities', () => {
    ...
    const ineligible = { reportType: 'SAST', severity: 'LOW', latestFlag: null };
