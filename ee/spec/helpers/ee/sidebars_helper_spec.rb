...
RSpec.describe ::SidebarsHelper, feature_category: :navigation do
  ...
  describe '#super_sidebar_context' do
    ...
    describe 'ai_search_available' do
      ...
      context 'when the feature_library_modal flag is disabled' do
        before do
          stub_feature_flags(feature_library_modal: false, feature_discovery_gemini_search: user)
          allow(group).to receive(:trial_active?).and_return(true)
        end
    ...
    context 'when user pinned multiple work items types and feature_library_modal is disabled' do
      using RSpec::Parameterized::TableSyntax
  ...
  describe '#super_sidebar_default_pins' do
    ...
    context 'when feature_library_modal is disabled' do
      ...
      it 'appends group_epic_list for group panel' do
        expect(helper.send(:super_sidebar_default_pins, 'group', test_user)).to eq(
          %w[group_issue_list group_merge_request_list group_epic_list]
        )
      end

      it 'returns CE defaults for project panel' do
        expect(helper.send(:super_sidebar_default_pins, 'project', test_user)).to eq(
    ...
    context 'when feature_library_modal is enabled' do
      ...
      it 'returns enriched CE defaults for project panel' do
        expect(helper.send(:super_sidebar_default_pins, 'project', test_user)).to eq(
