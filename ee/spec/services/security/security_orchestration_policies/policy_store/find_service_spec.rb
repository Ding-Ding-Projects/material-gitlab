...
RSpec.describe Security::SecurityOrchestrationPolicies::PolicyStore::FindService, :policy_store,
  ...
  subject(:service) { described_class.new(organization: organization, policy_id: policy.id) }
  ...
  describe '#execute' do
    ...
    context 'when the policy does not exist' do
      subject(:service) { described_class.new(organization: organization, policy_id: non_existing_record_id) }
