...
RSpec.describe Groups::OmniauthCallbacksController, :aggregate_failures, feature_category: :system_access do
  ...
  describe 'read-only organization enforcement exemption' do
    ...
    # Sign in an already-linked user: Gitlab::Auth::OAuth::User#save blocks
    # new-user creation during read-only (added in
    # https://gitlab.com/gitlab-org/gitlab/-/merge_requests/242419), so the
    # exemption only applies to plain sign-ins of existing identities.
    let(:user) { create_linked_user }

    before do
      # The omniauth middleware must validate the request, otherwise the
      # group_saml action raises ActionNotFound before enforcement runs.
    ...
    it 'does not block Group SAML callback POST requests with a read-only error' do
      post provider, params: { group_id: group }
    ...
    context 'with the organization_read_only_enforcement flag disabled' do
      ...
      it 'does not block Group SAML callback POST requests' do
        post provider, params: { group_id: group }
