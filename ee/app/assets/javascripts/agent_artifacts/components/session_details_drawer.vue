...
export default {
  ...
  watch: {
    // Ensure cursors and in-flight boundary state reset when session changes
    sessionId() {
      this.after = null;
      this.before = null;
      this.isFetchingBoundary = false;
...
<template>
  <mounting-portal mount-to="#contextual-panel-portal" append>
    <div data-testid="session-details-drawer" class="work-item-detail-panel gl-leading-reset">
      ...
      >
        <div v-if="selectedEvent" class="gl-flex gl-items-center">
          <gl-button
            variant="link"
            icon="arrow-left"
          ...
          <div class="gl-border-l gl-ml-3 gl-h-6 gl-border-l-section gl-pl-3">
            <gl-button-group>
              <gl-button
                v-gl-tooltip.bottom
                icon="chevron-left"
