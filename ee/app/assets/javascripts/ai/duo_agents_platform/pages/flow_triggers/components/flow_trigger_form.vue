...
import FlowTriggerEventsField from './flow_trigger_events_field.vue';
  ...
  methods: {
    setEventTypes(newStringValues) {
      this.eventTypes = newStringValues
        ...
        .filter((valueInt) => valueInt !== undefined);
    },
    onUserSelect(users) {
...
<template>
  <div>
    ...
    <gl-form class="gl-flex gl-grow gl-flex-col gl-gap-5" @submit.prevent="onSubmit">
      ...
      <div>
        <h2 class="gl-heading-3 gl-mb-1 gl-mt-3">{{ s__('DuoAgentsPlatform|Events') }}</h2>
        <p class="gl-mb-3 gl-text-subtle">
          {{ s__('DuoAgentsPlatform|When this trigger runs.') }}
        </p>
        <form-group
          ref="fieldEventTypes"
          #default="{ state, blur }"
          :field="fields.eventTypes"
          :field-value="eventTypes"
        >
          <flow-trigger-events-field
            :listbox-id="fields.eventTypes.id"
