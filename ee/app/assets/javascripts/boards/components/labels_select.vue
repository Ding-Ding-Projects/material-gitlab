...
<template>
  <div class="block labels labels-select-wrapper">
    ...
    <div class="gl-mb-2 gl-text-subtle" data-testid="selected-labels">
      ...
      <!-- eslint-disable vue/v-on-event-hyphenation -- DropdownValue is a shared component that emits the camelCase `onLabelRemove` event -->
      <dropdown-value
        v-else
        ...
        class="gl-mb-2"
        @label-removed="onLabelRemove"
      />
      <!-- eslint-enable vue/v-on-event-hyphenation -->
    </div>

    <!-- eslint-disable vue/v-on-event-hyphenation -- the shared labels_select_widget dropdown components emit the camelCase `toggleDropdownContentsCreateView`, `closeDropdown`, and `hideCreateView` events -->
    <dropdown-widget
      v-show="isEditing"
      ref="editDropdown"
