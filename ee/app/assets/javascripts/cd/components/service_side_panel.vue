...
import { SYNC_VARIANTS, SYNC_LABELS } from '../constants';
...
import ServiceSourceStatus from './service_source_status.vue';
    ...
    <dynamic-panel data-testid="service-side-panel" @close="$emit('close')">
      <template v-if="hasService" #header>
        <div class="gl-flex gl-w-full gl-items-center gl-justify-between gl-py-3">
          ...
          <div class="gl-flex gl-gap-2" data-testid="detail-header-badges">
      ...
      <div v-else-if="hasService" class="gl-pt-4" data-testid="detail-mode">
        <service-source-status
          ...
          :last-deployed="selectedService.lastDeployedAt"
          :deployed-by="selectedService.deployedBy"
          :sync="selectedService.sync"
          class="gl-mb-5"
        />
