...
// Section-level sync presentation for the service side panel
// (ServiceSourceStatus). Each entry bundles the icon, variant, and label so a
// new sync value cannot render with a missing/mismatched property. The compact
// header badge uses SYNC_LABELS ("Synced"/"Out of sync") instead; the divergent
// wording is intentional. See gitlab-org/gitlab#600804.
export const SYNC_STATUS = {
  synced: {
    icon: 'check-circle',
    variant: 'success',
    label: s__('ContinuousDeployment|In sync'),
  },
  'out-of-sync': {
    icon: 'warning',
    variant: 'warning',
    label: s__('ContinuousDeployment|Drift detected'),
  },
};
