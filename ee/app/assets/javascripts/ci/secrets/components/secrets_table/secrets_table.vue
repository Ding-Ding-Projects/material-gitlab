...
export default {
  ...
  inject: ['contextConfig', 'entitlement', 'fullPath', 'isOpenbaoHealthy', 'isReadOnly'],
  data() {
    return {
      secrets: [],
  ...
  apollo: {
    secrets: {
      ...
      skip() {
        return this.isTrialEligible || !this.isOpenbaoHealthy;
      },
      variables() {
        return {
...
<template>
  <div>
    ...
    <secrets-trial-empty-state v-else-if="entitlement && isTrialEligible" />
    <gl-empty-state
    ...
    >
      <template v-if="areActionsAllowed" #actions>
        <gl-button :to="$options.NEW_ROUTE_NAME" data-testid="empty-state-new-secret-button">
