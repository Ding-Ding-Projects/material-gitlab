...
export default {
  ...
  i18n: {
    ...
    locked: __('Locked'),
    cancel: __('Cancel'),
    lockedBy: __('Locked by %{locker}'),
    lockConfirmText: __('Are you sure you want to lock %{name}?'),
    unlockConfirmText: __('Are you sure you want to unlock %{name}?'),
  ...
  computed: {
    ...
    lockedByText() {
      return sprintf(this.$options.i18n.lockedBy, { locker: this.lockUser?.name }, false);
    },
    lockedAriaLabel() {
    ...
    lockConfirmText() {
      const template = this.isLocked
        ? this.$options.i18n.unlockConfirmText
        : this.$options.i18n.lockConfirmText;
...
<template>
  <div v-if="showComponent" class="gl-inline-flex">
    ...
    >
      ...
      <div v-if="lockUser" class="gl-px-4 gl-py-3" data-testid="lock-user-card">
        <div class="gl-mb-3">
