...
module Ai
  module DuoWorkflows
    class WorkflowCheckpointEventPresenter < Gitlab::View::Presenter::Delegated
      ...
      # Per-consumer gate for the GraphQL checkpoint reads (checkpoint,
      # compressedCheckpoint, duoMessages, lastDuoMessage): the workflow
      # reconstructs from blobs and the graphql flag is on, so these consumers
      # roll out independently of the others.
      def reconstruct_from_blobs_for_graphql?
        workflow = event.workflow
        return false unless workflow.reconstruct_from_blobs?

        Feature.enabled?(:dw_read_blobs_graphql, workflow.resource_parent) ||
          Feature.enabled?(:dw_read_blobs_graphql, workflow.resource_parent.root_ancestor)
      end
