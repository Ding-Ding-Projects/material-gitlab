...
class Groups::SsoController < Groups::ApplicationController
  ...
  # The group SAML sign-in landing (#saml) must remain available even when the
  # organization is read-only; it is a POST but only renders. #unlink is a real
  # identity write not needed to authenticate, so it stays enforced.
  # See https://gitlab.com/gitlab-org/gitlab/-/work_items/602813
  skip_before_action :enforce_read_only_organization, only: [:saml]
