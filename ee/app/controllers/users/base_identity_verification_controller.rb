...
module Users
  class BaseIdentityVerificationController < ApplicationController
    ...
    # Identity verification completes the sign-in gate, so it must remain
    # available even when the organization is read-only; otherwise a user cannot
    # finish authenticating to read it.
