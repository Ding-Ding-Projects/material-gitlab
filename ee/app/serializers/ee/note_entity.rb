...
module EE
  module NoteEntity
    ...
    prepended do
      ...
      expose :duo_session_id,
        if: ->(note, _) { note.duo_metadata.present? || note.duo_created_workflow_link.present? } do |note|
        note.duo_metadata&.workflow_id || note.duo_created_workflow_link&.workflow_id
      end
