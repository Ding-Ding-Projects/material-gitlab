...
module Ai
  module DuoWorkflows
    class Workflow < ::ApplicationRecord
      ...
      # Newest top-level header across the workflow, ordered by DWS thread_ts (a
      # time-ordered UUID; see CheckpointHeader.in_checkpoint_order). Scoped to the
      # top-level lineage to match the legacy `checkpoints.latest`, whose
      # `checkpoint_ns:` defaults to nil: without it a nested subagent's header could
      # outrank the flow's own newest checkpoint.
      #
      ...
      # so that is correct.
      def latest_checkpoint_header
        checkpoint_headers.for_checkpoint_ns(nil).in_checkpoint_order.last
      end
