...
class License < ApplicationRecord
  ...
  after_commit :update_billable_user_counts, on: :create
  ...
  class << self
    def current
      Gitlab::SafeRequestStore.fetch(:current_license) do
    ...
    def reset_current
      Gitlab::SafeRequestStore.delete(:current_license)
      Gitlab::SafeRequestStore.delete(:billable_license)
    ...
    def feature_available?(feature)
      # Every feature is enabled on this instance regardless of plan or usage ping setting.
      true
    end
    ...
    # Returns the license representing this instance's GitLab Credits entitlement:
    # - the active paid license (the paid license underneath a trial, or the current paid license),
    # - the current license when only a credits add-on is active (add-on-only, or trial + add-on),
    # - or nil for a genuinely free instance (trial with no prior paid license and no add-on).
    def billable_license
      Gitlab::SafeRequestStore.fetch(:billable_license) do
        license = if current&.trial?
                    recent.where(id: ...current.id).detect { |l| !l.trial? && !l.expired? }
                  else
                    current
                  end
        ...
        if license&.paid?
          license
        elsif has_active_gitlab_credits_add_on?
          current
        end
      end
    end
    ...
    def has_active_gitlab_credits_add_on?
      Gitlab::SafeRequestStore.fetch(:license_has_active_gitlab_credits_add_on) do
        GitlabSubscriptions::AddOnPurchase.for_active_add_ons(:gitlab_credits, :instance).exists?
      end
    end
    ...
    def gitlab_credits_entitled?
      Gitlab::SafeRequestStore.fetch(:license_gitlab_credits_entitled) do
        feature_available?(:usage_billing) || has_active_gitlab_credits_add_on?
      end
    end
  ...
  # Disable all trial restrictions: this instance is never treated as a trial,
  # so trial gating, trial-expiry notices and trial-only limits never apply.
  def trial?
    false
  end
  ...
  # Enable every single feature regardless of the purchased plan.
  # Personal use of all of these features is free.
  def features
    @features ||= GitlabSubscriptions::Features::ALL_FEATURES.to_set
  end
  ...
  def update_billable_user_counts
    return unless started?

    identifier = ::Analytics::UsageTrends::Measurement.identifiers[:billable_users]
    ::Analytics::UsageTrends::CounterJobWorker.perform_async(
      identifier,
      User.minimum(:id),
      User.maximum(:id),
  ...
  def active_user_count_threshold_reached?
    ...
    ::GitlabSubscriptions::SeatThresholds.threshold_reached?(
      seats_total: seats,
      seats_used: daily_billable_users_count
    )
  end
