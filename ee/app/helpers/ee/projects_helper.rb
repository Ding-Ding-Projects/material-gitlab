...
module EE
  module ProjectsHelper
    ...
    def duo_readiness_available?
      ::Feature.enabled?(:duo_agent_readiness_settings, current_user, type: :wip)
    end

    # The guards mirror the card's render conditions, so the runner reads are only paid
    # when the card actually renders.
    def duo_readiness_data(project)
      return {} unless duo_readiness_available?
      return {} if ::Ai::AmazonQ.connected?
      return {} unless can?(current_user, :update_duo_setting, project)

      readiness = ::Ai::DuoWorkflow::ProjectReadiness.new(project)

      {
        platformEnabled: readiness.platform_enabled?,
        runnerAvailable: readiness.runner_available?,
        # The client appends ?tab= from usableRunnerType. Not project_runners_path,
        # which redirects here and drops the query string.
        usableRunnerType: readiness.usable_runner_type,
