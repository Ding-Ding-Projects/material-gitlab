...
module Gitlab
  module DuoWorkflow
    ...
    class ChannelValuesReconstructor
      ...
      # One channel's blobs, deduped and oldest first by numeric version -- it's a
      # string column, so "10" must follow "9". Dedup identity is (thread_ts,
      # version), not version alone: DWS's channel_version counter is expected to
      # keep climbing across a compaction, but keying on thread_ts too means a
      # version number that did repeat across groups is kept instead of silently
      # dropped as a false-positive retry. A force_rewrite can store a
      # conversation delta and a compaction for the same (channel, version); `prefer`
      # picks which step_action wins, so the result never depends on insertion order
      # (https://gitlab.com/gitlab-org/gitlab/-/issues/604371). blobs_by_channel is
      # memoized, so #full_trace_channel_values folds every channel from one
      # group_by(&:channel) pass instead of re-scanning @blobs per channel.
      def ordered_blobs_for(channel, prefer:)
        blobs_by_channel.fetch(channel, [])
          .group_by { |blob| [blob.thread_ts, blob.version] }
          .transform_values { |rows| rows.find { |blob| blob.step_action == prefer } || rows.first }
          .values
          .sort_by { |blob| blob.version.to_i }
      end
