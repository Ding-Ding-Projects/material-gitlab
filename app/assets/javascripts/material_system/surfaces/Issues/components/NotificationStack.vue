<template>
  <div class="gl-mds-toasts" aria-live="polite" aria-atomic="false">
    <transition-group name="gl-mds-toast" tag="div" class="gl-mds-toasts__stack">
      <div
        v-for="item in visible"
        :key="item.id"
        class="gl-mds-toasts__item"
        :class="`gl-mds-toasts__item--${item.severity}`"
        role="status"
      >
        <mds-icon :name="iconFor(item.severity)" size="sm" />
        <div class="gl-mds-toasts__body">
        ...
        <material-button variant="text"
          ...
          :key="action.id"
          type="button"
          class="gl-mds-toasts__action"
          @click="runAction(item.id, action.id)"
        >
          {{ action.label }}
        </material-button>
        <material-button variant="text" type="button" class="gl-mds-toasts__dismiss" aria-label="Dismiss notification" @click="dismiss(item.id)">
...
.gl-mds-toasts__body { flex: 1; min-width: 0; }
.gl-mds-toasts__title { margin: 0; font-weight: 500; font-size: 13.5px; color: var(--gl-mds-onsurf); }
.gl-mds-toasts__message { margin: 2px 0 0; font-size: 12.5px; color: var(--gl-mds-onsurfv); line-height: 1.4; }

.gl-mds-toasts__action {
  align-self: center;
  background: none;
  border: none;
...
.gl-mds-toast-enter-active,
.gl-mds-toast-leave-active {
  transition: opacity 150ms ease, transform 150ms ease;
}

.gl-mds-toast-enter,
.gl-mds-toast-leave-to {
  opacity: 0;
