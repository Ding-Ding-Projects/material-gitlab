/**
 * Regex builder logic for the Plan surface's anchored popover. Reuses the
 * shared RegexBuilder primitive for pattern validity and sample matching,
 * and adds the pieces that primitive does not cover: a snippet palette, a
 * plain-English token explainer, and a live preview against real Plan text.
 */
...
export const SNIPPET_GROUPS = Object.freeze(
  [
    ...
    { name: 'Groups', items: [['( )', 'capture'], ['(?: )', 'non-capture'], ['(a|b)', 'alternation'], ['(?= )', 'lookahead'], ['(?! )', 'neg lookahead']] },
    { name: 'Recipes', items: [['REQ-\\d+', 'requirement id'], ['\\d+\\.\\d+', 'version number'], ['Sprint \\d+', 'sprint name'], ['(failed|missing)', 'unmet status']] },
...
/**
 * Evaluates a draft pattern against free-form sample text (for highlighting)
 * and against a flat corpus of strings (for the "matches" preview). Syntax
 * validity and sample matches are delegated to the shared RegexBuilder.
 */
export function evaluateRegexDraft({ pattern, flags, testText, corpus }) {
  ...
  try {
    ...
    preview = corpus
      ...
      .slice(0, 5);
    matchCount = `${preview.length} of ${corpus.length}`;
  } catch (_error) {
    // Handled by the validity check above.
  }

  return { valid: true, errorMessage: '', segments, captureGroups, preview, matchCount, explain: explainPattern(pattern) };
}
