  ...
  <section class="material-live-surface dp-surface" data-surface-id="surface.operate" :data-theme="dark ? 'dark' : 'light'">
    ...
    <div class="material-live-surface__heading">
      ...
      <nav class="material-live-surface__tabs" aria-label="Operate sections">
        <material-text-button v-for="tab in tabs" :key="tab.id" type="button" :aria-current="activeTab === tab.id ? 'page' : null" :aria-selected="activeTab === tab.id" @click="activeTab = tab.id">{{ tab.label }}</material-text-button>
      </nav>
    ...
    <main v-else class="material-live-surface__main">
      ...
      <div class="material-live-surface__card">
        <div v-for="row in rows" :key="row.id" class="material-live-surface__row">
          ...
          <material-text-button v-if="row.stopPath" type="button" :disabled="busy" @click="requestChange(row, 'stop')">Stop</material-text-button>
          <material-text-button v-if="activeTab === 'terraform' && endpoints.terraformAdmin" type="button" :disabled="busy" @click="requestChange(row, 'lock')">{{ row.status === 'locked' ? 'Force unlock' : 'Lock' }}</material-text-button>
        </div>
...
export default {
  name: 'OperateSurface',
  components: { ConfirmDialog, CommandPalette, RegexBuilderPopover, MaterialTextButton, MaterialTextField },
