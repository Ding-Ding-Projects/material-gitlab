      ...
      module Component
        module SecureReport
          ...
          def filter_by(token_name:, token_value:)
            wait_for_requests
            open_token_suggestions(token_name)
          ...
          # Focus the term input with JS to open the token list. Clicking it
          # is unreliable when the bar overflows or the input is zero-width.
          def open_token_suggestions(token_name)
            retry_until(sleep_interval: 1, message: "Show #{token_name} token suggestion") do
              page.execute_script(%q(document.querySelector('[data-testid="filtered-search-term-input"]')?.focus()))
              has_button?(token_name, wait: 2)
            end
          end
