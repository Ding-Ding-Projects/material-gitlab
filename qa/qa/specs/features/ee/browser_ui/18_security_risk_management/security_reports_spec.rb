    ...
    describe 'Security Reports' do
      ...
      it 'displays security reports in the project security dashboard' do
        ...
        EE::Page::Project::Secure::Show.perform do |vulnerability_report|
          vulnerability_report.filter_by(token_name: 'Scanner', token_value: 'gemnasium') do
            expect(vulnerability_report).to have_vulnerability dependency_scan_example_vuln
          end

          vulnerability_report.filter_by(token_name: 'Scanner', token_value: 'Trivy') do
            expect(vulnerability_report).to have_vulnerability container_scan_example_vuln
          end

          vulnerability_report.filter_by(token_name: 'Scanner', token_value: 'Brakeman') do
          ...
          vulnerability_report.filter_by(token_name: 'Scanner', token_value: 'GitLab API Security') do
            expect(vulnerability_report).to have_vulnerability dast_scan_example_vuln
          end

          vulnerability_report.filter_by(token_name: 'Scanner', token_value: 'Gitleaks') do
      ...
      it 'displays false positives for the vulnerabilities',
        ...
        EE::Page::Project::Secure::VulnerabilityReport.perform do |vulnerability_report|
          Support::Retrier.retry_on_exception(
            max_attempts: 2,
            sleep_interval: 3,
            reload_page: page,
            ...
            vulnerability_report.click_vulnerability(description: sast_scan_fp_example_vuln)
          end
        end

        EE::Page::Project::Secure::VulnerabilityDetails.perform do |vulnerability_details|
          expect(vulnerability_details).to have_false_positive_alert
        end
      end
